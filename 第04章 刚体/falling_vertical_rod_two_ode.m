%竖直杆滑倒的角速度和速度等与时间的关系(用两个一阶微分方程的数值解)
clear                                  %清除变量
t=0:0.1:100;                           %时间向量
th0=[pi/180/10;0];                     %初始角和初速度(初始角因微扰而很小)
options=odeset('Events','on');         %开启事件判断功能
[t,TH]=ode45('falling_vertical_rod_fun',t,th0,options);%解微分方程
%[t,TH,event]=ode45('P4_9_fun',t,th0,options);%解微分方程
omega=TH(:,2);                         %取角速度
th=TH(:,1);                            %取角度
theta=th*180/pi;                       %化为度度
...
leg1='\it\omegat\rm_0';                %第一个图例
leg2='\it\alphat\rm_0^2';              %第二个图例
leg3='\itv/v\rm_0';                    %第三个图例
leg4='\ita/g';                         %第四个图例
h=legend(leg1,leg2,leg3,leg4,0);       %加图例
set(h,'FontSize',fs)                   %放大图例
txt='\itt\rm_0=(2\itl/g\rm)^{1/2}';    %时间单位文本
txt=[txt ',\itv\rm_0=(2\itgl\rm)^{1/2}'];%连接速度单位文本
