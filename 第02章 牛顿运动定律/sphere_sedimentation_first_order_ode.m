%小球在水中沉降的运动规律(求一阶微分方程的数值解)
clear                                  %清除变量
tm=5;                                  %最大时间
dt=0.01;                               %时间间隔
t=0:dt:tm;                             %时间向量
f=inline('1-v','t','v');               %一阶微分方程的内线函数表达式
v0=0;                                  %初始条件
[tt,v]=ode23(f,t,v0);                  %求一阶微分方程的数值解
[t0 i]=min(abs(t-1));                  %1约化时间单位在时间向量中的下标
v1=v(i);                               %特征速度
x=cumtrapz(v)*dt;                      %用梯形法求深度
%-------------------------------------------------
figure                                 %创建图形窗口
...
xlabel('时间\itt/\tau','FontSize',fs)  %横坐标
ylabel('速度\itv/v\rm_T','FontSize',fs)%纵坐标
